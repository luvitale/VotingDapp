...
contract Voting {
  ...
  function Voting(bytes32[] candidatesName) public {
    for(uint i = 0; i < candidatesName.length; ++i) {
      candidates[candidatesName[i]] = true;
    }
  }

  // Total number of votes received by a candidate
  function totalVotes(bytes32 candidate) constant public returns (uint8) {
  ...
  function candidateIsValid(bytes32 candidate) constant public returns (bool) {
